...
**Équipe :** Patrick Mouliom, Christopher Gilleron, Jérémie Becker, Arnaud Hoarau, Karim Atebata.
...
3. **Extraction Sémantique Map-Reduce (Qwen) :** Déploiement du modèle LLM génératif `Qwen2.5-7B-Instruct`. Le pipeline interroge Supabase pour récupérer les 5 meilleurs blocs de texte (Retrieval) puis demande à Qwen de consolider ces informations (Reduce) sous forme de JSON structuré.
...
│  ├─ API v2 CDN      │──PDF─→│  localtunnel  │──PDF─→ │  ├─ PyMuPDF (Lecture PDF)     │
│  │  (Filtrage Intel.)│       │  (loca.lt)   │        │  ├─ BioBERT (Embedding 768d)  │
...
|-----------|-------------|------|
| **Scraping** | API v2 ClinicalTrials (Requests) | Filtrage intelligent des essais et téléchargement direct via CDN officiel |
...
## ⏭️ Perspectives d'Amélioration & Travaux Récents (Phase 4)
- **Phase 4 - Fine-Tuning QLoRA (Réalisé en MLOps) :** Pour augmenter le F1-Score sur l'extraction des entités CHIA, nous avons construit un script générant automatiquement un dataset conversationnel (132 études). Nous avons également rédigé les scripts d'entraînement complets pour Qwen 7B, optimisés `QLoRA` 4-bits (`paged_adamw_8bit`, `batch_size=1`, `max_seq_length=512`), garantissant une exécution fluide sans OOM sur les GPU Laptop de 6 Go VRAM.
- **Stockage :** Mise en place d'une politique d'auto-suppression Supabase (TTL/CRON) pour purger les fichiers PDF temporaires et limiter les coûts Cloud.
- **Performance :** Paralléliser l'envoi des PDF pour traiter plus de documents simultanément
- **Modèle :** Evaluer le modèle Qwen 7B fine-tuné contre le Qwen 7B de base.
